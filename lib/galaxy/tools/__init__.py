...
from galaxy.tool_util.cwl import needs_shell_quoting, shellescape, to_galaxy_parameters
...
class Tool(Dictifiable):
    ...
    def to_dict(self, trans, link_details=False, io_details=False, tool_help=False):
        ...
        regular_form = tool_class == Tool or isinstance(self, (DatabaseOperationTool, InteractiveTool, CwlCommandBindingTool))
...
class CwlCommandBindingTool(Tool):
    ...
    def exec_before_job(self, app, inp_data, out_data, param_dict=None):
        ...
        input_json = {k: v for k, v in input_json.items() if not (isinstance(v, dict) and v.get('class') == 'File' and v.get('location') == 'None')}
