...
class DrillDownSelectToolParameter(SelectToolParameter):
    ...
    def to_param_dict_string(self, value, other_values=None):
        ...
        def get_options_list(value):
            ...
            if base_option := get_base_option(value, options):
...
class FieldTypeToolParameter(ToolParameter):
    ...
    def to_json(self, value, app, use_security):
        """Convert a value to a string representation suitable for persisting"""
        log.info(f"to_json of {value} ({type(value)})")
        if value is None:
            return None
...
def history_item_to_json(value, app, use_security):
    ...
    if src is not None:
        ...
        rval = getattr(value, "extra_params", {})
        rval["id"] = app.security.encode_id(object_id) if use_security else object_id
        rval["src"] = src
        return rval
