...
class CwlToolsTestCase(integration_util.IntegrationInstance, unittest.TestCase):
    ...
    def test_wc(self):
        run_object = self.cwl_populator.run_cwl_artifact("wc-tool.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/wc-job.json")
        output1_content = self.dataset_populator.get_history_dataset_content(run_object.history_id)
        if not IS_OS_X:
    ...
    def test_wc2(self):
        run_object = self.cwl_populator.run_cwl_artifact("wc2-tool.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/wc-job.json")
        output1_content = self.dataset_populator.get_history_dataset_content(run_object.history_id)
    ...
    def test_wc4(self):
        run_object = self.cwl_populator.run_cwl_artifact("wc4-tool.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/wc-job.json")
        output1_content = self.dataset_populator.get_history_dataset_content(run_object.history_id)
        self.assertEqual(output1_content, "16")
    ...
    def test_env_tool2(self):
        run_object = self.cwl_populator.run_cwl_artifact("env-tool2.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/env-job.json")
        output1_content = self.dataset_populator.get_history_dataset_content(run_object.history_id)
        self.assertEqual(output1_content, "hello test env\n")

    @skip_without_tool("rename.cwl")
    def test_rename(self):
        run_object = self.cwl_populator.run_cwl_artifact("rename.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/rename-job.json")
        output1_content = self.dataset_populator.get_history_dataset_content(run_object.history_id)
        self.assertEqual(output1_content, whale_text())

    @skip_without_tool("optional-output.cwl")
    def test_optional_output(self):
        run_object = self.cwl_populator.run_cwl_artifact("optional-output.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/cat-job.json")
    ...
    def test_null_expression_1_1(self):
        run_object = self.cwl_populator.run_cwl_artifact("null-expression1-tool.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/empty.json")
        output1_content = self.dataset_populator.get_history_dataset_content(run_object.history_id)
        assert output1_content == '1', output1_content

    @skip_without_tool("null-expression1-tool.cwl")
    def test_null_expression_1_2(self):
        run_object = self.cwl_populator.run_cwl_artifact("null-expression1-tool.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/null-expression2-job.json")
        output1_content = self.dataset_populator.get_history_dataset_content(run_object.history_id)
    ...
    def test_null_expression_any_bad_1(self):
        ...
        run_object = self.cwl_populator.run_cwl_artifact("null-expression2-tool.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/null-expression1-job.json", assert_ok=False)
    ...
    def test_null_expression_any_bad_2(self):
        ...
        run_object = self.cwl_populator.run_cwl_artifact("null-expression2-tool.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/empty.json", assert_ok=False)
        self._assert_status_code_is(run_object.run_response, 400)

    @skip_without_tool("default_path.cwl")
    def test_default_path_override(self):
        run_object = self.cwl_populator.run_cwl_artifact("default_path.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/default_path_job.yml")
        output1_content = self.dataset_populator.get_history_dataset_content(run_object.history_id)
    ...
    def test_default_path(self):
        ...
        run_object = self.cwl_populator.run_cwl_artifact("default_path_custom_1.cwl", job={})
        stdout = self._get_job_stdout(run_object.job_id)
    ...
    def test_params1(self):
        run_object = self.cwl_populator.run_cwl_artifact("params.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/empty.json")
        output1_content = self.dataset_populator.get_history_dataset_content(run_object.history_id)
        assert output1_content == '"b b"', output1_content

    @skip_without_tool("parseInt-tool.cwl")
    def test_parse_int_tool(self):
        run_object = self.cwl_populator.run_cwl_artifact("parseInt-tool.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/parseInt-job.json")
        ...
        self.assertEqual(output1_content, '42')
        self.assertEqual(output1["extension"], "expression.json")

    @skip_without_tool("record-output.cwl")
    def test_record_output(self):
        run_object = self.cwl_populator.run_cwl_artifact("record-output.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/record-output-job.json")
